...
async function recoverToHome(driver: Driver): Promise<boolean> {
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      await driver.waitForSelector(HOME_ACTIVITY_TAB, { timeout: 3000 });
      return true;
    } catch (_error) {
      // continue recovery
    }
 ...
 */
describe('Production E2E: Multi-Network Swap Quotation Validation', function (this: Suite) {
  this.timeout(900000); // 15 minutes for multiple token pairs and network operations
          ...
          async ({ driver }: { driver: Driver }) => {
            ...
            for (
              ...
              try {
                ...
                const assertWithinPairDeadline = () => {
                  if (Date.now() > pairDeadline) {
                    throw new Error(
                      `[TIMEOUT] pair ${pairNumber}: ${pair.label} exceeded ${PER_PAIR_TIMEOUT_MS}ms`,
                    );
                  }
                };
