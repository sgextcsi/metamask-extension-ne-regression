 ...
 * Includes error recovery with back button if click doesn't navigate.
 *
 * @param driver - WebDriver instance
 * @param fromToken - Source token symbol
 * @param toToken - Destination token symbol
 * @throws Error only if recovery fails after back button attempts
 */
export async function openLatestBridgeActivityRecord(
  ...
  const detailPageIndicator = '[data-testid="transaction-breakdown-row-title"]';
  ...
  try {
    ...
    try {
      await driver.waitForSelector(detailPageIndicator, { timeout: 5000 });
      console.log('[BRIDGE] Activity detail page loaded successfully');
      return;
    } catch (_urlTimeout) {
      console.warn('[BRIDGE] Activity click did not navigate to detail page, attempting recovery...');
    }

    // Recovery: try back button and retry
    let recoverySucceeded = false;
    try {
      await navigateBack(driver, { timeout: 3000 });
      await driver.delay(500);
      await driver.clickElement(latestActivitySelector);
      await driver.delay(PROD_DELAYS.API_RESPONSE);
      await driver.waitForSelector(detailPageIndicator, { timeout: 5000 });
      recoverySucceeded = true;
      console.log('[BRIDGE] ✅ Activity detail page loaded after recovery');
      return;
    } catch (_recoveryError) {
      console.warn('[BRIDGE] Recovery attempt failed');
    }

    if (!recoverySucceeded) {
      throw new Error('Failed to open activity detail page even after recovery');
    }
  } catch (error) {
    const msg = `Failed to open activity detail page: ${error instanceof Error ? error.message : String(error)}`;
...
/**
 * Ensure low-value assets toggle is expanded before proceeding.
 * Checks if toggle is collapsed and expands it if needed.
 * Non-critical errors are logged as warnings only.
 *
 * @param driver - WebDriver instance
 */
export async function ensureLowValueAssetsExpandedForBridge(
  driver: Driver,
): Promise<void> {
  try {
    console.log('[BRIDGE] Checking low-value assets toggle state...');

    const toggleSelector = '[data-testid="low-value-asset-toggle"]';
    const toggleExists = await driver.isElementPresentAndVisible(toggleSelector, 2000).catch(() => false);

    if (!toggleExists) {
      console.log('[BRIDGE] ⓘ Low-value assets toggle not found (may not be needed), continuing...');
      return;
    }

    // Check if collapsed (aria-expanded="false")
    const toggle = await driver.findElement(toggleSelector).catch(() => null);
    if (!toggle) {
      console.log('[BRIDGE] ⓘ Toggle element found but cannot interact, continuing...');
      return;
    }

    const isExpanded = await toggle.getAttribute('aria-expanded').catch(() => null);
    console.log(`[BRIDGE] Toggle aria-expanded="${isExpanded}"`);

    if (isExpanded === 'false') {
      console.log('[BRIDGE] Toggle is collapsed, clicking to expand...');
      await toggle.click();
      await driver.delay(1000); // Wait for toggle animation
      console.log('[BRIDGE] ✅ Low-value assets expanded');
    } else {
      console.log('[BRIDGE] ✅ Low-value assets already expanded');
    }
  } catch (error) {
    const msg = `Warning: Could not ensure low-value assets expanded: ${error instanceof Error ? error.message : String(error)}`;
    console.warn(`[BRIDGE] ⚠️  ${msg}`);
    // Non-critical: don't throw, just warn and continue
  }
}

/**
 * Validate all required fields on the bridge activity detail page.
 * Implements soft-fail pattern: collects warnings but never throws.
 *
 * @param driver - WebDriver instance
 * @returns Validation results with all 7 fields and warnings (never throws)
 */
export async function validateBridgeActivityPageFields(
  driver: Driver,
): Promise<{
  allValid: boolean;
  warnings: string[];
  ...
  console.log('[BRIDGE-ACTIVITY] Starting bridge activity page fields validation...');
  ...
  // =========================================================================
  console.log('[BRIDGE-ACTIVITY] 1. Checking primary currency...');
  try {
    ...
    if (primaryText && primaryText.length > 0) {
      console.log(`[BRIDGE-ACTIVITY] ✅ Primary currency: ${primaryText}`);
      ...
      console.warn(`[BRIDGE-ACTIVITY] ⚠️  ${msg}`);
  ...
  // 2. VALIDATE STATUS
  // =========================================================================
  console.log('[BRIDGE-ACTIVITY] 2. Checking Status...');
  try {
    ...
    if (statusText && statusText.length > 0) {
      console.log(`[BRIDGE-ACTIVITY] ✅ Status: ${statusText}`);
  ...
  // =========================================================================
  console.log('[BRIDGE-ACTIVITY] 3. Checking Account...');
  try {
    ...
    if (accountText && accountText.length > 0) {
      console.log(`[BRIDGE-ACTIVITY] ✅ Account: ${accountText}`);
  ...
  // 4. VALIDATE NETWORK
  // =========================================================================
  console.log('[BRIDGE-ACTIVITY] 4. Checking Network...');
  try {
    ...
    if (networkText && networkText.length > 0) {
      console.log(`[BRIDGE-ACTIVITY] ✅ Network: ${networkText}`);
  ...
  // =========================================================================
  console.log('[BRIDGE-ACTIVITY] 5. Checking Transaction ID...');
  try {
    const txIdButtonXpath = '//*[@data-testid="transaction-id"]';
    const txIdButton = await driver.findElement({ xpath: txIdButtonXpath });

    console.log('[BRIDGE-ACTIVITY] Clicking transaction ID copy button...');
    await txIdButton.click();
    await driver.delay(500);

    console.log('[BRIDGE-ACTIVITY] Reading transaction ID from clipboard...');
    const txIdFromClipboard = await driver.getClipboardContent();
    const txId = txIdFromClipboard.trim();

    if (txId && txId.length > 0) {
      console.log(`[BRIDGE-ACTIVITY] ✅ Transaction ID (from clipboard): ${txId}`);
  ...
  // =========================================================================
  console.log('[BRIDGE-ACTIVITY] 6. Checking Network Fee...');
  try {
    ...
    if (networkFeeText && networkFeeText.length > 0) {
      console.log(`[BRIDGE-ACTIVITY] ✅ Network Fee: ${networkFeeText}`);
  ...
  // =========================================================================
  console.log('[BRIDGE-ACTIVITY] 7. Checking Total Amount...');
  try {
    ...
    if (totalAmountText && totalAmountText.length > 0) {
      console.log(`[BRIDGE-ACTIVITY] ✅ Total Amount: ${totalAmountText}`);
  ...
  // =========================================================================
  console.log(`[BRIDGE-ACTIVITY] ========== VALIDATION SUMMARY ==========`);
  console.log(`[BRIDGE-ACTIVITY] All Valid: ${results.allValid ? '✅ YES' : '⚠️ NO (SOFT FAIL - WARNINGS ONLY)'}`);
  console.log(`[BRIDGE-ACTIVITY] Total Warnings: ${results.warnings.length}`);
  if (results.warnings.length > 0) {
    console.log('[BRIDGE-ACTIVITY] Warnings (test will continue):');
    results.warnings.forEach((w, i) => {
      console.log(`[BRIDGE-ACTIVITY]   ${i + 1}. ${w}`);
    });
  }
