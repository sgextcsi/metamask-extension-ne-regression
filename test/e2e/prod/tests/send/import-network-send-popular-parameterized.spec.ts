...
function getSelectedPopularNetworks(): NetworkTestConfig[] {
...
describe('Production E2E: Popular Networks, Import Account and Send (Parameterized)', function (this: Suite) {
  ...
  const networks = getSelectedPopularNetworks();
  const allNetworkResults: SendTransactionResult[] = [];

  before(function () {
    console.log(
      `[PROD TEST] Running popular network send test for: ${networks
